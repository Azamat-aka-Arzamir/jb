...
/*
	...
	ssaoPass.kernelRadius = 16;      // AO spread
	ssaoPass.minDistance = 0.005;    // AO start distance
	ssaoPass.maxDistance = 0.1;      // AO end distance
	composer.addPass(ssaoPass);*/

	camera = new THREE.PerspectiveCamera(70, window.innerWidth / window.innerHeight, 0.2, 2000);
	camera.position.set(0, 15, 12);
	camera.lookAt(new THREE.Vector3(0, 0, -7));
	...
	scene.add(ambient);
	point = new THREE.PointLight(0xffff00, 0,5);
	scene.add(point);
...
function updatePhysics(deltaTime) {
	...
	var numSteps = deltaTime > 1.0 / 40.0 ? 2 : 1;
	onUpdate();
...
function renderExample() {
	...
	deltaTime = Math.min(deltaTime, 1.0 / 140.0);
	//deltaTime = Math.max(deltaTime, 1.0 / 40.0);
...
function createFloor(size = 50) {
	var shape = new Jolt.BoxShape(new Jolt.Vec3(size, 0.0000001, size), 0.05, null);
